...
* You can use a debugger or emulator/simulator and get valid Approov tokens if you [mark the signing certificate as being for development](https://approov.io/docs/latest/approov-usage-documentation/#development-app-signing-certificates). As a shortcut, when you are first setting up, you can add using the `latest` shortcut as discussed so that the `device ID` doesn't need to be extracted from the logs or an Approov token.
